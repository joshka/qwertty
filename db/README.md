...
## Width deviation tables (`db/width/<target>.toml`, `qdb width-probe`)

One file per target records how far the terminal's rendered advance for a grapheme cluster deviates
from the static unicode-width baseline — the db/-owned data the library's `width_of` embeds
(design [docs/design/09-width.md](../docs/design/09-width.md), hybrid mechanism). Generated by
`qdb width-probe --target <name>` (home + print + CPR advance), never hand-edited. `qdb
validate` guards them.

Run metadata mirrors the results schema (`target`, `version`, `version_source`, `adapter`,
`captured`, `runner`, `geometry`) plus `supports_2027` — whether the terminal recognised mode 2027
(grapheme clustering) via DECRQM. The probe **observes** 2027; it never enables it.

Each `[[cluster]]` row:

| Field           | Meaning                                                                              |
| --------------- | ------------------------------------------------------------------------------------ |
| `id`            | Stable corpus id (one of `qdb`'s `width::CORPUS` ids)                                |
| `text`          | The grapheme cluster, as a literal TOML string                                       |
| `unicode_width` | The static unicode-width baseline for `text`                                         |
| `advance`       | Measured column advance with 2027 off; **or** `no_cpr = true` (silence — data)       |
| `advance_2027`  | Measured advance with 2027 on — present only when `supports_2027` (or `no_cpr_2027`) |

A cluster whose `advance` differs from `unicode_width` is a real deviation for that terminal (the
classic set: ZWJ sequences, skin-tone modifiers, regional flags, VS16). Silence is recorded
`no_cpr`, never guessed.
