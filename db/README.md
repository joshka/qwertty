...
from the static unicode-width baseline — the db/-owned data the library's `width_of` embeds (design
`work/phase2/design/09-width.md`, hybrid mechanism). Generated by `qdb width-probe --target <name>`
(home + print + CPR advance), never hand-edited. `qdb validate` guards them.
