...
*unknown*, never *unsupported* (FM-C4). `Finding::evidence()` is a separate axis, `Evidence`:
...
- **`Evidence::Inferred { via }`** — no query exists for this capability; the value was guessed
  from an environment variable or heuristic named by `via` (FM-C12). Always used for
...
A consumer that only reads `.value()` sees the same tri-state `Option<T>` M3-S1 shipped; a consumer
that needs to tell "we asked and got told no" apart from "we never got an answer" reads
`.evidence()` as well. `Evidence` is `#[non_exhaustive]`, so a future evidence source (for example a
conformance-matrix lookup keyed by identity) can be added without an existing `match` becoming
non-exhaustive at compile time in a breaking way — match with a wildcard arm if you need to compile
against future versions.

## Terminal Identity (R-CAP-5)
...
1. **The XTVERSION reply**, when the probe bundle received one. `program_from_xtversion` does
   ...
   self-answering case (FM-C3). The full reply text becomes `identity.version` verbatim.
2. **`TERM_PROGRAM`**, when XTVERSION was silent: `iTerm.app`, `Apple_Terminal`, `vscode`,
   ...
   carried none.
3. **`TERM`**, consulted last because it is the least reliable signal (FM-C1: stale or
   out-of-date terminfo; FM-C2: TERM propagates over ssh to hosts whose database does not know
   it, and multiplexers set it to describe themselves): `xterm-kitty`, `alacritty`,
   `foot`/`foot-extra`, `tmux-256color`/`tmux`, `screen`/`screen-256color`.

Primary Device Attributes (DA1) is deliberately *not* an identity signal here: design 06 and FM-C4
call it a weak signal for features, and identity matching from DA1 param shape overlaps across
terminals even more than feature matching does, so this slice leaves `program` unresolved rather
than guess from DA1 alone.

### Multiplexer stack (FM-C3)
...
the user's terminal. Passthrough gating (tmux `allow-passthrough`, FM-M1/M2) reads `mux_stack`
rather than guessing from `program` alone.
...
## Environment Heuristics Have No Query (FM-C12)
...
- **`truecolor`** — 24-bit RGB SGR support. Inferred from `COLORTERM` ∈ `{truecolor, 24bit}` — the
  workaround FM-C1 documents for truecolor being otherwise inexpressible in terminfo.
...
when unanswered (FM-C7) — so a caller that detects `TERM=dumb` should skip
`probe_capabilities` entirely and treat every finding as unknown. This module has no opinion on
*when* a caller probes; it only guarantees that `identity_from_env` and the env-heuristic functions
are safe to call over any environment, `TERM=dumb` included, since they only read env vars and never
...
mode-2031-style theme-change events can change the answer mid-session. Deciding when a stale
`Capabilities` should be treated as invalid, and re-probing, is **M6's concern** — this slice only
produces the snapshot; there is no cache, no TTL, and no automatic re-probe.
