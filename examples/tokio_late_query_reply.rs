//! Open a Tokio-backed terminal session and treat a late query reply as ordinary input.
        ...
        Err(Error::QueryTimeout { timeout, .. }) => {
            ...
            match tokio::time::timeout(Duration::from_millis(250), session.next_event()).await {
                Ok(Ok(Event::Syntax(token))) => {
                    session
                        .text(format!(
                            "late reply arrived through next_event: {:?}\r\n",
                            token.as_bytes()
                        ))
                        .await?;
                }
                ...
                Err(_elapsed) => {
                    session
                        .text("no late reply arrived before the follow-up wait ended\r\n")
                        .await?;
                }
            }
        }
