//! Open a Tokio-backed terminal session and treat a cursor-position report as ordinary input during
//! a terminal-status query.
        ...
        Err(Error::QueryTimeout { timeout, .. }) => {
            ...
            match tokio::time::timeout(Duration::from_millis(250), session.next_event()).await {
                Ok(Ok(InputEvent::Csi(csi))) => {
                    session
                        .text(format!(
                            "cursor-position report arrived through next_event: {:?}\r\n",
                            csi.as_bytes()
                        ))
                        .await?;
                }
                ...
                Err(_elapsed) => {
                    session
                        .text("no wrong-report reply arrived before the follow-up wait ended\r\n")
                        .await?;
                }
            }
        }
