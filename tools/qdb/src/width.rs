...
/// Maps a db-width target slug to the `qwertty::caps::TerminalProgram` variant the library keys the
/// deviation table on. betamax hosts libghostty, so it is Ghostty; tmux does its own width math, so
/// it keys on the Tmux variant. A slug with no mapping is skipped in the generated table (its
/// deviations fall back to the static baseline, honestly).
fn slug_to_program(slug: &str) -> Option<&'static str> {
    match slug {
        "tmux" => Some("Tmux"),
        "betamax" => Some("Ghostty"),
        "kitty" => Some("Kitty"),
...
/// Escapes a cluster into a Rust string literal, rendering every non-ASCII-printable scalar as
/// `\u{..}` so the generated source is readable and the invisible clusters (ZWJ, VS, zero-width)
/// are unambiguous.
...
/// Renders the embedded `src/width_table.rs` for the library from the `db/width/*.toml` deviation
/// tables — the C2 embedding. Only clusters that actually deviate from the unicode-width baseline
/// (in either 2027 state) are emitted; the stable core stays the baseline. Deterministic: targets
/// sorted by slug, clusters in corpus order.
///
/// # Errors
///
/// Returns an error if a width table cannot be read or parsed.
pub fn render_rust_table(repo_root: &Path) -> Result<String, String> {
    ...
    out.push_str(
        "//! @generated by `qdb width-table` from `db/width/*.toml` — do not edit.\n\
         ...
         \x20   /// The grapheme cluster.\n\
         \x20   pub text: &'static str,\n\
         \x20   /// Measured advance with mode 2027 off/unset.\n\
         \x20   pub advance: usize,\n\
         \x20   /// Measured advance with mode 2027 on, when the terminal honours it.\n\
         \x20   pub advance_2027: Option<usize>,\n\
         }\n\n\
         /// A terminal's width deviations, keyed on its identity program.\n\
         pub(crate) struct Profile {\n\
         \x20   /// The terminal program this profile applies to.\n\
         \x20   pub program: TerminalProgram,\n\
         \x20   /// The clusters that deviate from the baseline on this terminal.\n\
         \x20   pub deviations: &'static [Deviation],\n\
         }\n\n\
    ...
    for path in paths {
        ...
        let value: toml::Value =
            toml::from_str(&text).map_err(|e| format!("parsing {name}: {e}"))?;
        let slug = value
            .get("target")
            .and_then(toml::Value::as_str)
            .unwrap_or("");
        let Some(program) = slug_to_program(slug) else {
            continue; // unmapped target — baseline fallback, no profile emitted
        };
        let clusters = value
            .get("cluster")
        ...
        for c in &clusters {
            let uw = c.get("unicode_width").and_then(toml::Value::as_integer);
            let advance = c.get("advance").and_then(toml::Value::as_integer);
            let advance_2027 = c.get("advance_2027").and_then(toml::Value::as_integer);
            let (Some(uw), Some(advance)) = (uw, advance) else {
                continue; // no baseline or no CPR — cannot assert a deviation
            };
            // Record only if some state deviates from the baseline.
            let off_deviates = advance != uw;
            let on_deviates = advance_2027.is_some_and(|a| a != uw);
            if !off_deviates && !on_deviates {
                continue;
            }
            let text = c.get("text").and_then(toml::Value::as_str).unwrap_or("");
            let on = advance_2027.map_or_else(|| "None".to_string(), |a| format!("Some({a})"));
            let _ = writeln!(
                rows,
                "        Deviation {{ text: {}, advance: {advance}, advance_2027: {on} }},",
                rust_str(text)
            );
        }
        if rows.is_empty() {
            continue; // every cluster matched the baseline — no profile needed
        }
        let _ = writeln!(
            out,
